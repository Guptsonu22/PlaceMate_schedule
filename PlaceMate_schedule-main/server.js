...
function myFunction() {
  ...
  Logger.log(thirtyMinutesAgo);
   ...
   const query='(from:"placement_cell_email" OR to:"placement_cell_email") is:unread'; // replace with actual placement email
...
const processedMessages = new Set();
  ...
  for(let thread of recentThreads) {
    ...
    for(let msg of messages){
      ...
      const fileMetadata = {
            'title': attachment.getName().replace(/\.(xlsx|xls)$/, ''),
            'mimeType': 'application/vnd.google-apps.spreadsheet'
        };
        
        // Convert to Google Sheet using Advanced Drive Service v2
...
function extractTime(text){
  ...
  // }
   ...
   // Support times with or without minutes (e.g., "5 pm" or "5:30 pm")
   const timePattern = /(?:by\s+)?(\d{1,2})(?::(\d{2}))?\s*([ap]m)/i;
